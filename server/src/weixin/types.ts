/**
 * Weixin iLink protocol types (mirrors proto: GetUpdatesReq/Resp, WeixinMessage, SendMessageReq).
...
export interface GetBotQrCodeReq {
  bot_type?: string;
  client_type?: number;
}
...
export interface GetQrCodeStatusReq {
  session_key?: string;
}
...
export interface GetQrCodeStatusResp {
  ...
  head_img_url?: string;
  expire_time?: number;
  /** When present, switch polling to this host for next request. */
  poll_host?: string;
}
...
// ========== HomeTale integration types ==========

export interface WeixinUserMapping {
  weixinUserId: string; // xxx@im.wechat
  roleId: string; // HomeTale role ID (dad/mom/child-xxx)
  /** true means role not yet identified, waiting for user to tell us */
  pending?: boolean;
  createdAt: string;
}

export interface WeixinSessionContext {
  weixinUserId: string;
  accountId: string;
  contextToken?: string;
  sessionId?: string; // HomeTale session ID
}
