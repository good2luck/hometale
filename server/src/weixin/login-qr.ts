...
import { redactToken } from "./util.js";
...
async function pollQRStatus(
  ...
  try {
    ...
    // 网关超时或其他网络错误，视为等待状态继续轮询
    logger.warn(
      `pollQRStatus: network/gateway error, will retry: ${String(err)}`
    );
    return { status: "wait" };
  }
}
      ...
      switch (statusResponse.status) {
        ...
        case "expired": {
          ...
          try {
            ...
            if (opts.onQrRefresh) {
              opts.onQrRefresh(qrResponse.qrcode_img_content);
            }
          } catch (refreshErr) {
            logger.error(
