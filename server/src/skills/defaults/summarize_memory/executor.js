  ...
  try {
    ...
    switch (action) {
      case 'summarize_today': {
        const unprocessed = await getUnprocessedConversations(roleId, today);
      ...
      case 'summarize_date': {
        ...
        const unprocessed = await getUnprocessedConversations(roleId, date);
        if (unprocessed.length === 0) {
      ...
      case 'summarize_today_and_update': {
        ...
        const unprocessedToday = await getUnprocessedConversations(roleId, today);
